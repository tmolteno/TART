...
class UVFitsGenerator(object):
    ...
    def gen_vis_table(self):
        ...
        hdu = fits.GroupsHDU(fits.GroupData(data, parnames=['UU', 'VV', 'WW', 'BASELINE', 'DATE'], bitpix=-32, \
    ...
    def gen_ant_table(self):
        ...
        c1 = fits.Column(name='ANNAME',     format='8A', array= [a['ANNAME']    for a in v.antennas])
        c2 = fits.Column(name='STABXYZ',    format='3D', array= [a['STABXYZ'] for a in v.antennas], unit='METERS') 
        c3 = fits.Column(name='NOSTA',        format='1J', array= [a['NOSTA']     for a in v.antennas])
        c4 = fits.Column(name='MNTSTA',     format='1J', array= [a['MNTSTA']    for a in v.antennas])
        c5 = fits.Column(name='STAXOF',     format='1E', array= [a['STAXOF']    for a in v.antennas], unit='METERS')
        c6 = fits.Column(name='POLTYA',     format='1A', array= [a['POLTYA']    for a in v.antennas])
        c7 = fits.Column(name='POLAA',        format='1E', array= [a['POLAA']     for a in v.antennas], unit='DEGREES')
        c8 = fits.Column(name='POLCALA',    format='3E', array= [a['POLCALA'] for a in v.antennas])
        c9 = fits.Column(name='POLTYB',     format='1A', array= [a['POLTYB']    for a in v.antennas])
        c10 = fits.Column(name='POLAB',     format='1E', array= [a['POLAB']     for a in v.antennas], unit='DEGREES')
        c11 = fits.Column(name='POLCALB', format='3E', array= [a['POLCALB'] for a in v.antennas])
        c0 = fits.Column(name='ORBPARM',    format='1D', array= [a['ORBPARM'] for a in v.antennas])

        return fits.BinTableHDU.from_columns([c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c0])
    ...
    def update_vis_header(self):
        ...
        self.vis_header.set('DATE-OBS', '%i-%02i-%02iT00:00:00.0'%(v.vis.timestamp.year, v.vis.timestamp.month, v.vis.timestamp.day))
        ...
        self.vis_header.set('PZERO5', int(tart_util.get_julian_date(v.vis.timestamp) + 0.5))
        ...
        if pol_typ < 0:
            self.vis_header.set('CDELT3', -1.)
        else:
            self.vis_header.set('CDELT3', 1.)

        self.vis_header.set('CRPIX4', 1.)    # something .... n_freq/2 +1
        self.vis_header.set('CDELT4', v.vis.config.get_bandwidth()) # freq delta
        self.vis_header.set('CRVAL4', v.vis.config.get_operating_frequency()) # center freq
    ...
    def update_ant_header(self):
        ...
        self.ant_header.set('TFORM5', '1E')         #     replace (J,E,D) with (1J,1E,1D) to match difmap requirements
        self.ant_header.set('TFORM7', '1E')         # # ra and dec of sun here! FIXME - move to fixed phase center in ra/dec
        ...
        self.ant_header.set('ARRAYY', arr_loc[1])    #    TART POSITION in ECEF coordinates
        ...
        self.ant_header.set('DEGPDY', 3.60985642988E+02) # Rotation per day

        self.ant_header.set('RDATE', "%d-%02d-%02dT00:00:00.0"%(v.vis.timestamp.year, v.vis.timestamp.month, v.vis.timestamp.day))
