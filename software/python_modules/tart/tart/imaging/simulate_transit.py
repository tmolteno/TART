...
def process_file(dirname, fname, orientation_degrees, spacing, sample_duration, simulate_vis=True):
  try:
    ...
    sec =  (nobs.timestamp - datetime.datetime.combine(date, datetime.time.min)).total_seconds()
    ...
    if (simulate_vis == True):
      ...
      sky, baselines, visibility = forward_map.forward_map(utc_date, location.Dunedin, horizon=5.0, antenna_locations=antenna_locations, sample_duration=sample_duration) # Remove things below an elevation of five degrees
    else:
      sky = False
      baselines = False
      visibility = [False]
...
def get_correlation(dirname, orientation_degrees, spacing, sample_duration, simulate_vis=True):
  ...
  if (True):
    ...
    try:
      for fname in files:
        if "data.pkl" in fname:
          resultList.append(p.apply_async(process_file, (dirname, fname, orientation_degrees, spacing, sample_duration, simulate_vis)))
    ...
    for fname in files:
      if "data.pkl" in fname:
        [date, hours,v,sv] = process_file(dirname, fname, orientation_degrees, spacing, sample_duration, simulate_vis)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Simulate the Sky and calculate simulated visibilities.')
  parser.add_argument('--data-dir', required=True, help="The filesystem path for the telescope data.")
  parser.add_argument('--orientation-angle', required=True, type=float, default=-22.0, help="The orientation of the array (degrees).")
  parser.add_argument('--optimize', action="store_true", help="Optimize the correlation between the simulated and measured visibilities.")
  parser.add_argument('--spacing', type=float, default=0.5, help="The spacing between antennas.")
  parser.add_argument('--sample-duration', type=float, default=1e-4, help="The length of time to simulate data for.")
  ...
  if (args.optimize):
    for theta in np.arange(orientation_degrees - 10.0, orientation_degrees+ 10.0, 1.0):
    ...
    plt.xlabel("Hours since UTC ")
    plt.savefig(os.path.join(dirname, "simulated_visibility_%f_%f.pdf" % (orientation_degrees, spacing)))
    plt.show()
