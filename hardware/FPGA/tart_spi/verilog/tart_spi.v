...
module tart_spi
  #( parameter ADDRESS = 12,
   ...
   //-------------------------------------------------------------------------
   reg                  bus_cycle = 0;
   wire                 bus_range = 0; // tart_addr[3:0] > 7 && tart_addr[3:0] < 11;
   wire                 bus_begin = 0; // byte_arrival && ack && tart_state == `TART_START && bus_range;
   ...
   always @(posedge clk)
     if (rst) wrap_addr <= #DELAY 0;
     else if (cyc && !we && ack)
       wrap_addr <= #DELAY tart_addr == ADDR_READ_DATA3;
     else if (data_sent)
       wrap_addr <= #DELAY 0;
