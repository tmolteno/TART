...
static int tart_mmap(struct file *f, struct vm_area_struct *vma)
{
  ...
  printk(KERN_INFO "tart_mmap\n");

  h = &head;
  len = 0; 
  while (len < vma->vm_end - vma->vm_start) {
    if (!mutex_trylock(&mem_mutex)) {
      schedule();
      continue;

    } else if (full == NULL) {
      mutex_unlock(&mem_mutex);
      /* TODO: if something has happened to dma_task this should
...
static int dma_thread(void *data)
{
  ...
  while (!kthread_should_stop()) {
	...
	printk(KERN_ERR "TART DMA stopped.\n");
	return -1;

      } else if (!mutex_trylock(&mem_mutex)) {
	continue;
      }
	...
	printk(KERN_ERR "TART DMA submit error error\n");
	return -1;
      }
	...
	printk(KERN_ERR "TART DMA returned completion status of:");
	switch (status) {
...
static long tart_unlocked_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {
    case TART_IOCTL_DMA_FLUSH:
      while (!mutex_trylock(&mem_mutex))
	schedule();

      for (n = &free; *n != NULL; n = &(*n)->next)
	;
	...
	printk(KERN_ERR "TART copy_from_user failed for 0x%p\n", 
	    (void *) arg);
	return -EFAULT;
      }
	...
	printk(KERN_ERR "TART copy_to_user failed for 0x%p\n", 
	    (void *) arg);
	return -EFAULT;
      }
...
static int tart_of_probe(struct platform_device *pdev)
{
  ...
  if (IS_ERR(regs)) {
    printk(KERN_ERR "TART failed to remap resource!\n");
    return -1;
  }
  ...
  if (chan == NULL || IS_ERR(chan)) {
    printk(KERN_ERR "TART failed to get dma channel\n");
    return PTR_ERR(chan);
  }

  np = of_parse_phandle(pdev->dev.of_node, "memory-region", 0);
  if (!np) {
    printk(KERN_ERR "TART failed to get memory-region!\n");
    return -1;
  }

  rc = of_address_to_resource(np, 0, &reserved);
  if (rc) {
    printk(KERN_ERR "TART No memory address assigned to the region\n");
    return -1;
  }
...
static int __init tart_init(void)
{
  ...
  dev = device_create(class, NULL, 
      MKDEV(TART_MAJOR_NUM, 0), 
      NULL, DEVICE_NAME);
