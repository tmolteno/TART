...
module ddr (Dq, Dqs, Addr, Ba, Clk, Clk_n, Cke, Cs_n, Ras_n, Cas_n, We_n, Dm);
    ...
    `include "Micron/ddr_parameters.v"
...
`endif
    ...
    begin
        ...
        if (Data_out_enable === 1'b1) begin
            ...
            if (Debug) begin
                $display ("At time %t READ : Bank = %d, Row = %d, Col = %d, Data = %d", $time, Bank_addr, Rows_addr, Cols_addr, Dq_out);
            end
    ...
    begin
        ...
        if (Data_in_enable === 1'b1) begin
            ...
            if (Sys_clk) begin
                ...
                if (~&dm_fall) begin
                    if (Debug) begin
                        $display ("At time %t WRITE: Bank = %d, Row = %d, Col = %d, Data = %d", $time, Bank_addr, Rows_addr, Cols_addr, Dq_buf[DQ_BITS-1:0]);
                    end
                end
    ...
    begin
        ...
        if (Write_enable === 1'b1) begin
            ...
            // display debug message
            if (Debug) begin
                $display ("At time %t WRITE: Bank = %d, Col = %d", $time, Ba, {Addr [ADDR_BITS - 1 : 11], Addr [9 : 0]});
            end
