...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--speed", default=32, type=float, help="Specify the SPI CLK speed (in MHz)"
    )
    parser.add_argument(
        "--bramexp", default=11, type=int, help="exponent of bram depth"
    )
    parser.add_argument(
        "--debug", action="store_true", help="operate telescope with fake antenna data"
    )
    parser.add_argument(
        "--blocksize", default=24, type=int, help="exponent of correlator block-size"
    )
    ...
    parser.add_argument(
        "--monitor", action="store_true", help="monitor for visibilities"
    )
    parser.add_argument(
        "--correlate", action="store_true", help="perform a single correlation"
    )
    ...
    parser.add_argument(
        "--source", default=0, type=int, help="antenna source to calibrate"
    )
    parser.add_argument(
        "--capture", action="store_true", help="just enable the data-capture unit"
    )
    parser.add_argument(
        "--centre", action="store_true", help="enable the clock-recovery unit"
    )
    parser.add_argument(
        "--setdelay", default=0, type=int, help="set signal phase-delay"
    )
    parser.add_argument("--diagnostic", action="store_true", help="run diagnostic")

    # TODO:
    parser.add_argument(
        "--phases", default="", type=str, help="file of antenna phase-delays"
    )
    ...
    if args.monitor or args.correlate:
        ...
        while True:
            if args.verbose:
                ...
                ts_now = time.time() - ts
                print(
                    " Reordered (   permuted) visibilities (@t = %g):\n%s (sum = %d)"
                    % (ts_now, viz[pp], sum(viz))
                )
            if args.correlate:
                tart.close()
                exit(0)
